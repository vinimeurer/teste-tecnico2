"""Extraction module for veículos (vehicles) data.

Reads raw CSV data from the veículos source and persists it
...
VEICULOS_SCHEMA = StructType(
    [
        ...
        StructField("capacidade_kg", IntegerType(), True),
        StructField("capacidade_paletes", IntegerType(), True),
        StructField("km_atual", IntegerType(), True),
        StructField("status", StringType(), True),
        StructField("data_ultima_revisao", DateType(), True),
    ]
)


def read_raw_data(spark: SparkSession, path: str) -> DataFrame:
    """Read raw veículos CSV data.
...
def validate_and_cast_schema(df: DataFrame) -> DataFrame:
    ...
    """
    for field in VEICULOS_SCHEMA.fields:
        if field.name not in df.columns:
            raise ValueError(
                f"Coluna obrigatória '{field.name}' ausente em veiculos."
            )
    return df.select(
        [
...
def extract_to_bronze(spark: SparkSession, config: dict | None = None) -> str:
    """Extract veículos raw data and persist in bronze layer as Parquet.
